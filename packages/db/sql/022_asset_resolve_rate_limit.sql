-- Asset URL resolution mints short-lived delivery URLs. It is read-only, but
-- must still be bounded so one authenticated user cannot create unlimited
-- gateway traffic or Supabase reads.

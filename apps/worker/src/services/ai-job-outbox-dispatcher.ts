...
export function startAiJobOutboxDispatcher(params: {
  ...
  const dispatch = async (trigger: "startup" | "interval") => {
    ...
    const execute = async () => {
      ...
      if (error) throw new Error(`Unable to claim AI job outbox records: ${error.message}`);
      ...
      try {
        for (const row of rows) {
          try {
            ...
            const queuedJob = await queue.add(AI_JOB_QUEUE_EVENT_NAME, payload, {
              jobId: row.ai_job_id,
            });
            ...
            if (markError || !marked) {
              throw new Error(markError?.message ?? "AI_JOB_OUTBOX_MARK_DISPATCHED_FAILED");
            }
            ...
            const message = error instanceof Error ? error.message : "AI_JOB_OUTBOX_DISPATCH_FAILED";
    ...
    try {
      ...
      logger.error("AI job outbox dispatcher failed", { trigger, error });
      void notifyOperationalAlert({

...
export const uploadLibraryAssets = async (params: {
  ...
  for (const file of params.files) {
    ...
    const sourceImage = sharp(fileBuffer, { failOn: "none" }).rotate();
    ...
    const originalBuffer = await encodeImage(fileBuffer, targetFormat);
    const thumbBuffer = await encodeImage(fileBuffer, targetFormat, { width: 360, height: 360 });
    const previewBuffer = await encodeImage(fileBuffer, targetFormat, { width: 1600, height: 1600 });
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        mime_type: targetFormat === "png" ? "image/png" : "image/jpeg",
...
export const buildLibraryAssetRecord = (asset: LibraryAssetRow) => ({
  id: asset.id,

...
function createIntentGuidance(intent: LandscapeIntent, context: DetectedLandscapeContext, mode: EnhanceMode) {
  ...
  switch (intent) {
    ...
    default:
      return `Clarify and strengthen the user's landscape request for ${targetArea} without changing the original design intent. Keep the prompt concise, spatially controlled, and suitable for ${mode.replaceAll("_", " ")}.`;
  }
}

...
const actions = [
  ...
  { label: "Region Edit", icon: Brush, action: "region" },
...
export default function FloatingQuickPanel({ viewportZoom = 1, onTool, onMultiAngle, onRealityCheck, onToast }: FloatingQuickPanelProps) {
  ...
  return (
    <aside
      className="floating-quick-panel absolute z-[100] w-44 rounded-2xl border border-[var(--canvas-theme-border)] bg-[var(--canvas-theme-surface-panel)] p-2 shadow-xl shadow-[var(--canvas-theme-shadow)] backdrop-blur"
      style={{
    ...
    >
      <p className="px-2 pb-1.5 text-[10px] font-black uppercase tracking-[0.16em] text-[var(--canvas-theme-text-muted)]">Quick Panel</p>
        ...
        {actions.map((action) => {
          ...
          return (
            <button
              ...
              if ("action" in action && action.action === "region") onTool("region");
              if ("action" in action && ["expand", "style", "crop", "rotate", "customize"].includes(action.action)) onToast(`${action.label} applied`);
            }}
              className="flex items-center gap-2 rounded-xl px-2.5 py-2 text-left text-xs font-bold text-[var(--canvas-theme-text)] transition hover:bg-[var(--canvas-theme-hover)]"
            >
